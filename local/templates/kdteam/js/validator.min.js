! function(t) {
    "use strict";
    var e = function(a, r) {
        for (var i in this.$element = t(a), this.options = r, r.errors = t.extend({}, e.DEFAULTS.errors, r.errors), r.custom)
            if (!r.errors[i]) throw new Error("Missing default error message for custom validator: " + i);
        t.extend(e.VALIDATORS, r.custom), this.toggleSubmit(), this.$element.on("input.bs.validator change.bs.validator focusout.bs.validator", t.proxy(this.validateInput, this)), this.$element.on("submit.bs.validator", t.proxy(this.onSubmit, this)), this.$element.find("[data-match]").each(function() {
            var e = t(this),
                a = e.data("match");
    ...
    function a(a) {
        return this.each(function() {
            var r = t(this),
                i = t.extend({}, e.DEFAULTS, r.data(), "object" == typeof a && a),
                o = r.data("bs.validator");
            (o || "destroy" != a) && (o || r.data("bs.validator", o = new e(this, i)), "string" == typeof a && o[a]())
        })
    }
    e.INPUT_SELECTOR = ':input:not([type="submit"], button):enabled:visible', e.DEFAULTS = {
        delay: 500,
        html: !1,
        disable: !0,
        custom: {},
    ...
    }, e.prototype.runValidators = function(a) {
        var r = [],
            i = t.Deferred(),
            o = this.options;

        function n(t) {
            return a.data(t + "-error") || a.data("error") || "native" == t && a[0].validationMessage || o.errors[t]
        }
        return a.data("bs.validator.deferred") && a.data("bs.validator.deferred").reject(), a.data("bs.validator.deferred", i), t.each(e.VALIDATORS, t.proxy(function(t, e) {
            if ((a.data(t) || "native" == t) && !e.call(this, a)) {
                var i = n(t);
                !~r.indexOf(i) && r.push(i)
            }
        }, this)), !r.length && a.val() && a.data("remote") ? this.defer(a, function() {
            var e = {};
            e[a.attr("name")] = a.val(), t.get(a.data("remote"), e).fail(function(t, e, a) {
                r.push(n("remote") || a)
            }).always(function() {
                i.resolve(r)
            })
        ...
        return this.options.delay = 0, this.$element.find(e.INPUT_SELECTOR).trigger("input.bs.validator"), this.options.delay = t, this
        ...
        return !!this.$element.find(e.INPUT_SELECTOR).filter(function() {
            return !!(t(this).data("bs.validator.errors") || []).length
        }).length
    }, e.prototype.isIncomplete = function() {
        return !!this.$element.find(e.INPUT_SELECTOR).filter("[required]").filter(function() {
        ...
        this.options.disable && t('button[type="submit"], input[type="submit"]').filter('[form="' + this.$element.attr("id") + '"]').add(this.$element.find('input[type="submit"], button[type="submit"]')).toggleClass("disabled", this.isIncomplete() || this.hasErrors())
    }, e.prototype.defer = function(e, a) {
        if (a = t.proxy(a, this), !this.options.delay) return a();
        window.clearTimeout(e.data("bs.validator.timeout")), e.data("bs.validator.timeout", window.setTimeout(a, this.options.delay))
    }, e.prototype.destroy = function() {
        return this.$element.removeAttr("novalidate").removeData("bs.validator").off(".bs.validator"), this.$element.find(e.INPUT_SELECTOR).off(".bs.validator").removeData(["bs.validator.errors", "bs.validator.deferred"]).each(function() {
            var e = t(this),
    ...
    t.fn.validator = a, t.fn.validator.Constructor = e, t.fn.validator.noConflict = function() {
